  ...
  switch (platform) {
    ...
    case 'linux-x64':
      return isNightly(version)
        ? 'boa-x86_64-unknown-linux-gnu'
        : 'boa-linux-amd64';
    case 'win32-x64':
      return isNightly(version)
        ? 'boa-x86_64-pc-windows-msvc.exe'
        : 'boa-windows-amd64.exe';
    case 'darwin-arm64':
...
class BoaInstaller extends Installer {
  ...
  static resolveVersion(version) {
    if (version === 'latest') {
      ...
      return fetch(
        'https://api.github.com/repos/boa-dev/boa/releases/tags/nightly',
      )
        .then((r) => r.json())
  ...
  getDownloadURL(version) {
    return `https://github.com/boa-dev/boa/releases/download/${
      getReleaseTag(version)
    }/${getFilename(version)}`;
  }
  ...
  async extract() {
    ...
    return copyFileSync(
      this.downloadPath,
      join(this.extractedPath, binaryName),
    );
  }
